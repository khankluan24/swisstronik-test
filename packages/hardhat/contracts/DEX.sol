...
contract DEX {
	...
	function init(uint256 tokens) public payable returns (uint256) {
		...
		// require(msg.sender.balance >= msg.value, "Insufficent ether balance");
		...
		// require(sent, "Failed to transfer tokens to the contract");
		require(
			token.transferFrom(msg.sender, address(this), tokens),
			"Failed to transfer tokens to the contract"
		);
	...
	function ethToToken() public payable returns (uint256 tokenOutput) {
		...
		require(ethInput > 0, "ETH amount must be greater than 0");
	...
	function tokenToEth(uint256 tokenInput) public returns (uint256 ethOutput) {
		...
		(bool sentEther, ) = msg.sender.call{ value: ethOutput }("");
		require(sentEther, "Failed to send Ether to the sender");
	...
	function deposit() public payable returns (uint256 tokensDeposited) {
		...
		tokensDeposited = ((ethInput * tokenReserve) / ethReserve) + 1;
	...
	function withdraw(
		...
		uint256 callerLiquidity = liquidity[msg.sender];
		require(callerLiquidity >= amount, "Not enough liquidity");
		uint256 tokenReserve = token.balanceOf(address(this));
		uint256 ethReserve = address(this).balance;
		...
		require(success, "Failed to send Ether to you");
		require(
			token.transfer(msg.sender, tokenOutput),
			"Failed to transfer tokens to the contract"
		);
